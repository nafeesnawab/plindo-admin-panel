...
export default function TermsConditionsPage() {
	...
	const { data: currentVersion, isLoading } = useQuery({
		...
		queryFn: () => legalService.getTermsConditions(),
	});

	const { data: history } = useQuery({
		queryKey: ["legal-terms-history"],
		queryFn: () => legalService.getTermsHistory(),
	});
	...
	const updateMutation = useMutation({
		mutationFn: (publish: boolean) => legalService.updateTermsConditions({ content, publish }),
		onSuccess: (_, publish) => {
			toast.success(publish ? "Terms published successfully" : "Draft saved");
			queryClient.invalidateQueries({ queryKey: ["legal-terms"] });
			queryClient.invalidateQueries({ queryKey: ["legal-terms-history"] });
		},
				...
				<TabsContent value="history" className="mt-4">
					<Card>
						<CardHeader>
							...
							<CardDescription>Previous versions of Terms & Conditions</CardDescription>
						</CardHeader>
						<CardContent>
							<Table>
								<TableHeader>
