...
import { Building2, Percent, Save, Users } from "lucide-react";
...
export default function CommissionSettingsPage() {
	...
	const updateMutation = useMutation({
		mutationFn: () =>
			settingsService.updateCommissionSettings({
				customerCommission,
				partnerCommission,
			}),
		onSuccess: () => {
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
					...
					<CardContent className="space-y-4">
						...
						<div className="p-4 bg-muted/50 rounded-lg">
							<p className="text-sm text-muted-foreground">
								Example: For a €20 service, customer pays{" "}
								<strong>€{(20 * (1 + customerCommission / 100)).toFixed(2)}</strong>
							</p>
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>Percentage deducted from partner earnings on each booking</CardDescription>
					</CardHeader>
					<CardContent className="space-y-4">
						<div className="space-y-2">
							<Label>Commission Rate (%)</Label>
						...
						<div className="p-4 bg-muted/50 rounded-lg">
							<p className="text-sm text-muted-foreground">
								Example: For a €20 service, partner receives{" "}
								<strong>€{(20 * (1 - partnerCommission / 100)).toFixed(2)}</strong>
							</p>
							<p className="text-sm text-muted-foreground mt-1">
				...
				<CardContent className="pt-6">
					<div className="flex items-center justify-between">
						<div>
							...
							<p className="text-sm text-muted-foreground">Customer commission + Partner commission</p>
						</div>
						<div className="text-right">
							<p className="text-2xl font-bold text-green-600">
								€{((20 * customerCommission) / 100 + (20 * partnerCommission) / 100).toFixed(2)}
							</p>
							<p className="text-sm text-muted-foreground">{customerCommission + partnerCommission}% total</p>
						</div>
					</div>
				</CardContent>
			</Card>
