...
partnerApplications.set("rejected@demo.com", {
	id: faker.string.uuid(),
	status: "rejected",
	rejectionReason: "Business license document was not readable. Please resubmit with a clearer copy.",
...
const registerPartner = http.post(`/api${PartnerAuthApi.Register}`, async ({ request }) => {
	...
	try {
		...
		partnerApplications.set(businessInfo.email, {
			...
			password: "partner123", // Default password for demo
		});
...
const getApplicationStatus = http.get(`/api${PartnerAuthApi.ApplicationStatus}`, async ({ request }) => {
...
const checkEmailAvailability = http.get(`/api${PartnerAuthApi.CheckEmail}`, async ({ request }) => {
	const url = new URL(request.url);
	const email = url.searchParams.get("email");
...
const partnerSignIn = http.post(`/api${PartnerAuthApi.Login}`, async ({ request }) => {
	...
	if (application.status !== "approved") {
		return HttpResponse.json({
			status: 10003,
			message: "Your account is not active. Please check your application status.",
			data: {
				partner: {
					id: application.id,
					businessName: application.businessName,
...
export const partnerAuthHandlers = [registerPartner, getApplicationStatus, checkEmailAvailability, partnerSignIn];
