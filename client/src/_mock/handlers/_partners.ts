...
const generatePartner = (status: "pending" | "active" | "suspended") => {
	const services = faker.helpers.arrayElements(carWashServices, { min: 2, max: 5 });
	return {
		...
		rating: status === "pending" ? null : faker.number.float({ min: 3.5, max: 5, fractionDigits: 1 }),
		totalBookings: status === "pending" ? 0 : faker.number.int({ min: 50, max: 2000 }),
		completionRate: status === "pending" ? null : faker.number.int({ min: 85, max: 100 }),
		totalEarnings: status === "pending" ? 0 : faker.number.float({ min: 5000, max: 50000, fractionDigits: 2 }),
		...
		documents: [
			...
			{ name: "Insurance Certificate", url: "#", verified: faker.datatype.boolean() },
		],
	};
};

// Generate mock data
const pendingPartners = Array.from({ length: 12 }).map(() => generatePartner("pending"));
const activePartners = Array.from({ length: 25 }).map(() => generatePartner("active"));
const suspendedPartners = Array.from({ length: 8 }).map(() => generatePartner("suspended"));
const allPartners = [...pendingPartners, ...activePartners, ...suspendedPartners];

// Get pending applications
export const getPendingPartners = http.get(`/api${PartnerApi.Pending}`, ({ request }) => {
...
// Get active partners
export const getActivePartners = http.get(`/api${PartnerApi.Active}`, ({ request }) => {
	const url = new URL(request.url);
	const page = Number.parseInt(url.searchParams.get("page") || "1");
	const limit = Number.parseInt(url.searchParams.get("limit") || "10");
...
export const getSuspendedPartners = http.get(`/api${PartnerApi.Suspended}`, ({ request }) => {
	const url = new URL(request.url);
	const page = Number.parseInt(url.searchParams.get("page") || "1");
	const limit = Number.parseInt(url.searchParams.get("limit") || "10");
	const start = (page - 1) * limit;
...
export const getPartnerDetails = http.get(`/api/partners/:id`, ({ params }) => {
	...
	const reviews = Array.from({ length: faker.number.int({ min: 5, max: 15 }) }).map(() => ({
		id: faker.string.uuid(),
...
// Reactivate partner
export const reactivatePartner = http.post(`/api/partners/:id/reactivate`, () => {
	return HttpResponse.json({
		status: ResultStatus.SUCCESS,
